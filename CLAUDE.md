...
- **`lg:` と 1200px デザインのギャップ注意**: `lg:` は 1024px から発火するが、デザイン基準は 1200px。デスクトップ列の合計幅が 1200px 付近になる場合、固定幅 (`w-[Npx]`) + `flex-wrap` を組み合わせると 1024〜1159px で行落ちが発生する。代わりに **`flex-1 min-w-0`** を使って縮小可能にする。
